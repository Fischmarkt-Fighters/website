...
import { Shield, Swords, Cross, Loader2, Crown, Zap, Target, Trophy, BarChart3, Play } from 'lucide-react';
import { useTeamRoster } from '../../hooks/useFaceitClub';
import type { Player } from '../../hooks/useFaceitClub';

function PlayerCard({ player, idx }: { player: Player, idx: number }) {
  ...
  const rank = stats?.bestRank;
  const winrate = stats?.winrate || 0;
  const endorsement = stats?.endorsement?.level || 1;
          ...
          <div className="p-6 pb-4 flex justify-between items-center gap-4">
            <div className="flex items-center gap-4 min-w-0 flex-1">
                <div className="relative shrink-0">
                    <div className="w-16 h-16 rounded-xl border-2 border-white/10 group-hover:border-white/30 overflow-hidden bg-black transition-all duration-500 shadow-xl">
                        {(stats?.avatar || player.avatar) ? (
                            <img src={stats?.avatar || player.avatar} alt={player.nickname} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                        ) : (
                            <div className="w-full h-full flex items-center justify-center bg-zinc-900 text-zinc-700 font-black text-2xl italic uppercase">
                ...
                <div className="min-w-0 flex-1 flex flex-col justify-center">
                    <div className="flex items-center gap-1.5 mb-2">
                        <h3 className="text-xl md:text-2xl font-black font-sans italic uppercase tracking-tighter text-white leading-none whitespace-nowrap pr-2">
                            {player.nickname}
                        </h3>
                        <div className="flex items-center gap-1 shrink-0">
                            {player.isLeader && <Crown className="w-4 h-4 text-yellow-500 fill-yellow-500/20" />}
                            <div className="flex items-center justify-center w-5 h-5 rounded-full border border-white/20 bg-white/5 text-[10px] font-mono font-bold text-zinc-400 group-hover:text-white group-hover:border-white/40 transition-all shadow-inner" title="Endorsement Level">
                                {endorsement}
                            </div>
                        </div>
                    </div>
                    ...
                    {player.roles && player.roles.length > 0 && (
                        <div className="flex gap-2 flex-wrap">
                            {player.roles.map(r => (
                                <div key={r} className="flex items-center gap-1.5 bg-black/40 border border-white/5 rounded-md px-2 py-1 shadow-sm">
                                    <span className="text-zinc-400 scale-75 origin-left">{getRoleIcon(r)}</span>
                                    <span className="text-[8px] font-mono font-bold text-zinc-400 uppercase tracking-widest leading-none mt-px">{r}</span>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            {rank?.rank_icon ? (
                <div className="relative shrink-0 flex flex-col items-center">
                    <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest mb-1 font-bold">Best Rank</span>
                    <img src={rank.rank_icon} alt="rank" className="w-14 h-14 object-contain drop-shadow-white group-hover:scale-110 transition-transform duration-500" />
                    <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-md border border-white/20 px-1.5 rounded-full text-[8px] font-mono font-bold text-white uppercase whitespace-nowrap">
                        {rank.division} {rank.tier}
                    </div>
                </div>
            ) : (
                <div className="relative shrink-0 w-14 h-14 flex flex-col items-center justify-center">
                    <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest mb-1 font-bold whitespace-nowrap">Best Rank</span>
                    <span className="text-[10px] font-mono text-zinc-600 uppercase italic mt-1">Unranked</span>
                </div>
            )}
          </div>

          <div className="px-6 text-center">
              <span className="text-[8px] font-mono text-zinc-600 uppercase tracking-[0.4em] block border-y border-zinc-800/50 py-1">Lifetime Statistics</span>
          </div>
            ...
            <div className="flex justify-center gap-4">
                {topHeroes.length > 0 ? topHeroes.map(([heroName, heroData]: any) => (
                    <div key={heroName} className="flex flex-col items-center group/hero">
                        ...
                        <span className="text-[8px] font-mono uppercase text-zinc-600 mt-2 tracking-widest group-hover/hero:text-white transition-colors">{heroName}</span>
                    </div>
                )) : (
                        ...
                        <div key={role.id}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {players.map((player, idx) => (
                                    <PlayerCard key={`${role.id}-${player.nickname}`} player={player} idx={idx} />
                                ))}
                            </div>
                        </div>
                    );
